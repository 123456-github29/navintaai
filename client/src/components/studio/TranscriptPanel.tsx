  ...
  if (isError || !data || !data.captions || data.captions.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <div>
          <p className="text-sm text-white/50 mb-1">No transcript found</p>
    ...
    <div className="flex-1 overflow-y-auto px-4 py-4">
      {data.language && (
        <div className="flex items-center gap-1.5 mb-4">
          <span className="text-[10px] text-white/35 uppercase tracking-wider">Language</span>
          <span className="text-[10px] px-1.5 py-0.5 bg-white/[0.06] text-white/50 rounded font-mono">
            {data.language}
          </span>
          <span className="text-[10px] text-white/35 ml-auto">{captions.length} segments</span>
        </div>
      )}
        ...
        {captions.map((caption, idx) => (
          ...
          >
            <div className="flex items-start pt-0.5 shrink-0">
              <span className="flex items-center gap-1 text-[11px] text-white/35 font-mono group-hover:text-blue-500/70 transition-colors">
