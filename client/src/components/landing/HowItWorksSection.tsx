...
const steps = [
  { number: "01", title: "Describe your goal", visual: "prompt" },
  { number: "02", title: "Get a shot-by-shot plan", visual: "plan" },
  { number: "03", title: "Record with AI guidance", visual: "record" },
  { number: "04", title: "Export polished video", visual: "export" },
];
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        {["Instagram Reel", "9:16", "30s", "Professional"].map((tag) => (
          <span key={tag} className="px-3 py-1.5 rounded-full bg-white/[0.06] text-[11px] text-white/50 font-medium">{tag}</span>
        ))}
      </div>
      <button className="w-full bg-white text-black rounded-full py-3 text-sm font-semibold">Generate plan →</button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2.5">
      {[
        ...
        <div key={shot.num} className="flex items-center gap-3 bg-white/[0.03] rounded-xl px-4 py-3">
          <span className="w-7 h-7 rounded-full bg-emerald-500/15 text-emerald-400 text-[11px] font-bold flex items-center justify-center shrink-0">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button className="flex-1 py-2.5 rounded-xl text-xs font-medium text-white/60 bg-white/[0.06]">Retake</button>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {["9:16", "1:1", "16:9"].map((fmt) => (
          <span key={fmt} className="flex-1 text-center py-2 rounded-lg bg-white/[0.06] text-[10px] text-white/50 font-medium">{fmt}</span>
        ))}
      </div>
      <button className="w-full py-3 rounded-full text-sm font-semibold text-black bg-white">Export video →</button>
    </div>
  );
}
...
export default function HowItWorksSection() {
  ...
  return (
    <section ref={sectionRef} id="showcase" className="min-h-screen flex items-center relative overflow-hidden py-20" style={{ background: "#000000" }}>
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full" style={{ background: "radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 60%)", filter: "blur(80px)" }} />
      </div>

      <div className="max-w-6xl mx-auto px-6 w-full relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-white mb-4">Four steps. One video.</h2>
          <p className="text-lg text-white/40 max-w-lg mx-auto">Watch your idea become a polished, publish-ready video.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            {steps.map((step, i) => (
              <button
                ...
                className={`w-full text-left p-6 rounded-2xl transition-all duration-500 ${
                  i === activeStep ? "bg-white/[0.05]" : "bg-transparent hover:bg-white/[0.02]"
                }`}
              >
                <div className="flex items-center gap-4">
                  <span className={`text-sm font-mono font-bold transition-colors duration-500 ${i === activeStep ? "text-indigo-400" : "text-white/20"}`}>{step.number}</span>
                  <span className={`text-lg font-semibold transition-colors duration-500 ${i === activeStep ? "text-white" : "text-white/30"}`}>{step.title}</span>
                </div>
                {i === activeStep && (
          ...
          <div className="relative">
            <div className="rounded-2xl bg-[#111111] p-6 min-h-[480px] flex flex-col justify-center" style={{ boxShadow: "0 20px 60px rgba(0,0,0,0.5), 0 0 40px rgba(99,102,241,0.05)" }}>
